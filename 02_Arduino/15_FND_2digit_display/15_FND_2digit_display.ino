...
void loop() {
  ...
  digitalWrite(FndSel[3], HIGH);
    ...
    for(i=0; i<8; i++){ //0x3f
      if(FndValue[1] & (0x01<<i)){ //각 비트들 값에 따라서 LED ON/OFF
        digitalWrite(FndData[i],HIGH);
      } else {
  ...
  digitalWrite(FndSel[3], HIGH);
    ...
    for(i=0; i<8; i++){ //0x3f
      if(FndValue[2] & (0x01<<i)){ //각 비트들 값에 따라서 LED ON/OFF
        digitalWrite(FndData[i],HIGH);
      } else {
        digitalWrite(FndData[i],LOW);
      }
  ...
  digitalWrite(FndSel[3], HIGH);
    ...
    for(i=0; i<8; i++){ //0x3f
      if(FndValue[3] & (0x01<<i)){ //각 비트들 값에 따라서 LED ON/OFF
        digitalWrite(FndData[i],HIGH);
      } else {
        digitalWrite(FndData[i],LOW);
      }
  ...
  digitalWrite(FndSel[3], LOW);
    ...
    for(i=0; i<8; i++){ //0x3f
      if(FndValue[4] & (0x01<<i)){ //각 비트들 값에 따라서 LED ON/OFF
        digitalWrite(FndData[i],HIGH);
      } else {
        digitalWrite(FndData[i],LOW);
      }
